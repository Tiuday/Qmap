...
export const FluidCanvas: React.FC<FluidCanvasProps> = ({
  ...
  const setBoundary = useCallback((boundary: number, field: Float32Array) => {
    ...
    for (let i = 1; i < size - 1; i++) {
      ...
      field[i + (size - 1) * size] =
        boundary === 2 ? -field[i + (size - 2) * size] : field[i + (size - 2) * size];
    }
    for (let j = 1; j < size - 1; j++) {
      field[j * size] = boundary === 1 ? -field[j * size + 1] : field[j * size + 1];
      field[j * size + (size - 1)] =
        boundary === 1 ? -field[j * size + (size - 2)] : field[j * size + (size - 2)];
    }
    field[0] = 0.33 * (field[1] + field[size] + field[size + 1]);
    field[size - 1] = 0.33 * (field[size - 2] + field[2 * size - 1] + field[2 * size - 2]);
    field[(size - 1) * size] =
      0.33 * (field[(size - 2) * size] + field[(size - 1) * size + 1] + field[(size - 2) * size + 1]);
    field[size * size - 1] =
      0.33 * (field[size * size - 2] + field[(size - 1) * size - 1] + field[(size - 2) * size - 1]);
  }, []);

  const linearSolve = useCallback(
    (boundary: number, field: Float32Array, field0: Float32Array, a: number, c: number) => {
      const { size } = gridRef.current;
      const cRecip = 1.0 / c;
      for (let iter = 0; iter < 8; iter++) {
        for (let j = 1; j < size - 1; j++) {
          for (let i = 1; i < size - 1; i++) {
            ...
            field[index] =
              (field0[index] +
                a * (field[index - 1] + field[index + 1] + field[index - size] + field[index + size])) *
              cRecip;
          }
        }
  ...
  const diffuse = useCallback(
    (boundary: number, field: Float32Array, field0: Float32Array, diff: number, dt: number) => {
      const { size } = gridRef.current;
  ...
  const project = useCallback(
    (velocX: Float32Array, velocY: Float32Array, p: Float32Array, div: Float32Array) => {
      const { size } = gridRef.current;
      for (let j = 1; j < size - 1; j++) {
        for (let i = 1; i < size - 1; i++) {
          const index = j * size + i;
          div[index] =
            (-0.5 * (velocX[index + 1] - velocX[index - 1] + velocY[index + size] - velocY[index - size])) /
            size;
          p[index] = 0;
        }
      }
    ...
    (
      ...
      for (let j = 1; j < size - 1; j++) {
        for (let i = 1; i < size - 1; i++) {
          ...
          field[index] =
            s0 * (t0 * field0[j0 * size + i0] + t1 * field0[j1 * size + i0]) +
            s1 * (t0 * field0[j0 * size + i1] + t1 * field0[j1 * size + i1]);
        }
      }
      setBoundary(boundary, field);
    },
  ...
  const step = useCallback(() => {
    const { size, velocityX, velocityY, velocityX0, velocityY0, density, density0, temperature } =
      gridRef.current;
    const dt = 0.016;
    ...
    for (let i = 0; i < size * size; i++) {
      ...
      if (temperature[i] > 0.1) {
        ...
        if (y > 0) velocityY[i] -= temperature[i] * 0.01;
      }
    }
  ...
  const render = useCallback(() => {
    ...
    for (let j = 0; j < height; j++) {
      for (let i = 0; i < width; i++) {
        ...
        if (d > 0.01) {
          // Purple-cyan fluid palette matching Qmap brand
          const hue = 260 + temp * 60;
          const saturation = 0.85 + temp * 0.15;
          const lightness = d * 0.55 + temp * 0.3;
    ...
    if (opacity > 0.5) {
      ...
      glowGradient.addColorStop(0, '#a78bfa');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setOpacity((prev) => {
        ...
        if (Math.abs(diff) < stepSize) { clearInterval(interval); return target; }
        return prev + (diff > 0 ? stepSize : -stepSize);
      });
    }, 16);
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: '20px',
        overflow: 'hidden',
        background: 'linear-gradient(145deg, rgba(18,18,28,0.95) 0%, rgba(10,10,20,0.98) 100%)',
        border: '1px solid rgba(139, 92, 246, 0.15)',
        cursor: 'pointer',
        transition: 'all 0.5s cubic-bezier(0.23, 1, 0.32, 1)',
        transform: isHovered ? 'scale(1.01) translateY(-3px)' : 'scale(1) translateY(0)',
        boxShadow: isHovered
          ? '0 30px 60px rgba(0,0,0,0.5), 0 12px 30px rgba(139,92,246,0.2)'
    ...
    >
      <div
        ...
        style={{
          background: 'radial-gradient(circle at 30% 20%, rgba(139,92,246,0.1), transparent 60%)',
      ...
      <canvas
        ...
        style={{
          ...
          height: '100%',
          display: 'block',
          filter: isHovered ? 'contrast(1.15) brightness(1.05)' : 'contrast(1.05)',
